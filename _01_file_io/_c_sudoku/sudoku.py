...
#      '005010300]
sudoku_grid_list = []
random_number = 10*random.randint(0, 49)+1
def open_sudoku_file(sudoku_grids_file):
    with open(file=sudoku_grids_file, mode='r') as file_handle:
        var = file_handle.readlines()
        for i in range(9):
            number = str(var[random_number+i])
            sudoku_grid_list.append(number)
            print(random_number)
            print(sudoku_grid_list)
...
def on_save_button(grid_list, save_file='saved.txt'):
    with open(file="new_file.txt", mode='w') as file_handle:
        file_handle.write(str(grid_list))
    return None
